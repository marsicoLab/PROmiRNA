...
void SequenceDB::WriteClusters2( const char *db, const char *newdb, const Options & options, std::vector<std::string> lines )
{
	...
	if( fout == NULL ) bomb_error( "file opening failed" );
...
void SequenceDB::DRead( const char *file, const Options & options, std::vector<std::string> lines )
{
	Sequence one;
	Sequence des;
...
//	if( fin == NULL ) bomb_error( "Failed to open the database file" );
	...
	for(int y=0;y<lines.size()+1;y++){
		//buffer[0] = '>';
		//if ( one.size == 0) break;
		//cerr << "line=" << lines[y] << endl;
		//	}
		if(y<lines.size())
			strcpy(buffer, lines[y].c_str());

	if (buffer[0] == '>' || buffer[0] == '@' || (one.size)) {
...
void SequenceDB::Read( const char *file, const Options & options )
{
	...
	while (not feof( fin ) || one.size) { /* do not break when the last sequence is not handled */
		...
		if( buffer[0] == '+' ){
			...
			one.des_begin = offset - len;
			one.tot_length += len;              // count first line
			//cerr << "offset-len=" << offset - len << endl;
			int i = 0;
			if( des.data[i] == '>' || des.data[i] == '@' || des.data[i] == '+' ) i += 1;
			if( des.data[i] == ' ' or des.data[i] == '\t' ) i += 1;
			if( options.des_len and options.des_len < des.size ) des.size = options.des_len;
